...
// Copied from: https://github.com/pontem-network/BigNum/blob/main/sources/u256.move
...
module samm::bignum {
    ...
    // Errors.
    /// When can't cast `BigNum` to `u128` (e.g. number too large).
    const ECAST_OVERFLOW: u64 = 0;

    /// When trying to get or put word into BigNum but it's out of index.
    ...
    /// Total words in `BigNum` (64 * 4 = 256).
    const WORDS: u64 = 4;

    /// When both `BigNum` equal.
    const EQUAL: u8 = 0;
    ...
    /// Double `BigNum` used for multiple (to store overflow).
    struct DBigNum has copy, drop, store {
        v0: u64,
        v1: u64,
        v2: u64,
        v3: u64,
    ...
    // Public functions.
    /// Adds two `BigNum` and returns sum.
    public fun add(a: BigNum, b: BigNum): BigNum {
        let ret = zero();
        let carry = 0u64;
    ...
    /// Convert `BigNum` to `u128` value if possible (otherwise it aborts).
    public fun as_u128(a: BigNum): u128 {
        assert!(a.v2 == 0 && a.v3 == 0, ECAST_OVERFLOW);
        ((a.v1 as u128) << 64) + (a.v0 as u128)
    }

    /// Convert `BigNum` to `u64` value if possible (otherwise it aborts).
    public fun as_u64(a: BigNum): u64 {
        assert!(a.v1 == 0 && a.v2 == 0 && a.v3 == 0, ECAST_OVERFLOW);
        a.v0
    }

    /// Compares two `BigNum` numbers.
    public fun compare(a: &BigNum, b: &BigNum): u8 {
        let i = WORDS;
        while (i > 0) {
            i = i - 1;
            let a1 = get(a, i);
    ...
    /// Returns a `BigNum` from `u64` value.
    public fun from_u64(val: u64): BigNum {
        from_u128((val as u128))
    }

    /// Returns a `BigNum` from `u128` value.
    public fun from_u128(val: u128): BigNum {
    ...
    /// Multiples two `BigNum`.
    public fun mul(a: BigNum, b: BigNum): BigNum {
        let ret = DBigNum {
            v0: 0,
            v1: 0,
            v2: 0,
    ...
    /// Subtracts two `BigNum`, returns result.
    public fun sub(a: BigNum, b: BigNum): BigNum {
        let ret = zero();
    ...
    /// Divide `a` by `b`.
    public fun div(a: BigNum, b: BigNum): BigNum {
        let ret = zero();
    ...
    public fun shr(a: BigNum, shift: u8): BigNum {
        let ret = zero();
    ...
    public fun shl(a: BigNum, shift: u8): BigNum {
        let ret = zero();
    ...
    /// Returns `a` AND `b`.
    public fun and(a: &BigNum, b: &BigNum): BigNum {
        let ret = zero();
    ...
    /// Returns `a` OR `b`.
    public fun or(a: &BigNum, b: &BigNum): BigNum {
        let ret = zero();
    ...
    /// Returns `a` XOR `b`.
    public fun xor(a: &BigNum, b: &BigNum): BigNum {
        let ret = zero();
    ...
    /// Returns `BigNum` equals to zero.
    public fun zero(): BigNum {
        BigNum {
            v0: 0,
            v1: 0,
            v2: 0,
    ...
    public fun get(a: &BigNum, i: u64): u64 {
        if (i == 0) {
            a.v0
        } else if (i == 1) {
            a.v1
    ...
    /// Get word from `DBigNum` by index.
    fun get_d(a: &DBigNum, i: u64): u64 {
        if (i == 0) {
            a.v0
        } else if (i == 1) {
            a.v1
    ...
    /// Put new word `val` into `BigNum` by index `i`.
    fun put(a: &mut BigNum, i: u64, val: u64) {
        if (i == 0) {
            a.v0 = val;
        } else if (i == 1) {
            a.v1 = val;
    ...
    /// Put new word into `DBigNum` by index `i`.
    fun put_d(a: &mut DBigNum, i: u64, val: u64) {
        if (i == 0) {
            a.v0 = val;
        } else if (i == 1) {
            a.v1 = val;
    ...
    /// Convert `DBigNum` to `BigNum`.
    fun du256_to_u256(a: DBigNum): (BigNum, bool) {
        let b = BigNum {
            v0: a.v0,
            v1: a.v1,
            v2: a.v2,
    ...
    /// Converts `vector<u8>` `a` to a `BigNum`.
    public fun from_bytes(a: &vector<u8>): BigNum {
    ...
    /// Converts `BigNum` `a` to a `vector<u8>`.
    public fun to_bytes(a: &BigNum): vector<u8> {
